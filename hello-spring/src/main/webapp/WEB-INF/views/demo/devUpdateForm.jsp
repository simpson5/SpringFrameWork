	...
	<form name="devFrm"
		...
		<div class="form-group row">
			...
			<div class="col-sm-10">
				<div class="form-check form-check-inline">
				  <input
				  	class="form-check-input" type="checkbox" name="lang" id="Java" value="Java"
				  	${Arrays.toString(dev.lang).contains('Java') ? 'checked' : ''}>
				  <label class="form-check-label" for="Java">Java</label>
				</div>
				<div class="form-check form-check-inline">
				  <input
				  	class="form-check-input" type="checkbox" name="lang" id="C" value="C"
				  	${Arrays.toString(dev.lang).contains('C') ? 'checked' : ''}>
				  <label class="form-check-label" for="C">C</label>
				</div>
				<div class="form-check form-check-inline">
				  <input
				  	class="form-check-input" type="checkbox" name="lang" id="Javascript" value="Javascript"
				  	${Arrays.toString(dev.lang).contains('Javascript') ? 'checked' : ''}>
				  <label class="form-check-label" for="Javascript">Javascript</label>
				</div>
				<div class="form-check form-check-inline">
				  <input
				  	class="form-check-input" type="checkbox" name="lang" id="Python" value="Python"
				  	${Arrays.toString(dev.lang).contains('Python') ? 'checked' : ''}>
 ...
 * 수정폼 유효성검사
 */
console.log("${Arrays.toString(dev.lang)}");
$(document.devFrm).submit((e) => {
	//1. 이름은 한글 2글자 이상이어야 한다.
	var $name = $("name");
	if(!/&[가-힣]{2,}$/.test($name.vla())){
		alert("없어");
		e.preventDefault();
	}
