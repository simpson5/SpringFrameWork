		...
		<div id="menu-container" class="text-center">
			...
			*/
			...
			$("#menuSearchFrm").submit(e => {
				...
				const $restaurant = $frm.find("[name = restaurant]");
				const $name = $frm.find("[name = name]");
				const $price = $frm.find("[name = price]");
				const $type = $frm.find("[name = type]");
				const $taste = $frm.find("[name = taste]");

				$.ajax({
					url : "${pageContext.request.contextPath}/menus/" + id,
					success(data){
						console.log(data);
			...
			$("#menuUpdateFrm").submit(e => {
				...
				$.ajax({
					url : "${pageContext.request.contextPath}/menus/update",
					method : 'PUT',
					data : JSON.stringify(menu),
					contentType: "application/json; charset=utf-8",
					success(data){
...
function displayResultTable(id, data){
	...
	//mybatis session.selectList는 데이터 없는 경우, 빈 list를 리턴
	if(data.length > 0){
		$(data).each((i, menu) =>{
		...
		html += "<tr><td colspan='6'>검색 결과 없음</td></tr>"
	}
	html += "</table>";
